    ...
    def declarations(self):  # 构造声明节点的方法
        ...
        if self.current_token.type == VAR:  # 如果当前记号为变量
            ...
            while self.current_token.type == ID:  # 遍历变量名称
                ...
                self.eat(SEMI)  # 验证分号
        while self.current_token.type == PROCEDURE:  # 当前记号类型是过程时
...
class SymbolTable:  # 添加符号表类
    ...
    def __str__(self):
        ...
        return f'Symbols:{self._symbols}'
...
class DrawASTtreeImg(NodeVisitor):  # 添加符号表生成器类
    ...
    def visit_Block(self, node):
        ...
        for declaration in node.declarations:
            decldata.append(self.visit(declaration))
    ...
    def visit_ProcedureDecl(self, node):  # 定义过程声明的绘图方法
        ProcedureDecldata = self.visit(node.block_node)  # 过程声明节点由Block节点组成
        Blockdata = {"children": ProcedureDecldata, "name": "ProcedureDecl\n" + node.name}
        return Blockdata
    ...
    def drawastimg(self):  # 绘制AST语法树图形
        ...
        Asttree.render(r'out/L14AstTree.html')  # 绘制AST语法树图形
...
class SymbolTableBuilder(NodeVisitor):  # 添加符号表生成器类
    ...
    def visit_VarDecl(self, node):  # 添加访问变量声明节点方法
        type_name = node.type_node.name  # 获取变量类型名称
        symbol_type = self.symbol_table.lookup(type_name)  # 获取变量类型
        var_name = node.var_node.name  # 获取变量名称
    ...
    def visit_NoOp(self, node):  # 添加访问空语句的节点
        pass  # 不做任何处理（空语句之后的语句可能包含符号，需要跳过空语句）

    def visit_BinOp(self, node):  # 添加访问二元运算符方法（二元运算中可能包含符号）
    ...
    def visit_UnaryOp(self, node):  # 添加访问一元运算符方法
...
END.  {Part12}
    ...
    print('AST语法树结构图:out/L14AstTree.html')
