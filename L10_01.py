...
class Parser:  # 语法分析器
    ...
    def header(self):  # 程序头区
        self.eat(PROGRAM)
        self.eat(ID)
        self.eat(SEMI)

    def varline(self):  # 变量声明区每一行
        node = Variable(self.current_token)  # 获取第1个变量定义节点
        nodes = [node]  # 变量节点列表
        self.eat(ID)  # 验证变量名称
        while (self.current_token.type == COMMA):  # 遍历完所有逗号间隔的每个变量定义节点
            self.eat(COMMA)  # 验证逗号
            node = Variable(self.current_token)  # 获取变量节点
            nodes.append(node)  # 累计所有的变量
            self.eat(ID)  # 验证变量名称
        self.eat(COLON)  # 验证冒号
        vartype = self.current_token.value  # 获取数据类型节点（INTEGER OR REAL）

        for node in nodes:  # 存储所有的变量：名称和类型
            VAR_TYPE[node.name] = vartype

        self.eat(DATATYPE)  # 验证数据类型关键字
        self.eat(SEMI)  # 验证分号

    def var(self):  # 变量声明区
        self.eat(VAR)  # 验证变量声明区关键字VAR
        while (self.current_token.type != BEGIN):  # 保证读完变量区所有的行
            self.varline()  # 每一行变量解析
...
class Interpreter(NodeVisitor):
    ...
    def visit_Assign(self, node):  # 访问赋值语句节点
        ...
        var_type = '<class ' + "'" + VAR_TYPE[var_name] + "'" + '>'
