...
class Parser:  # 语法分析器
    ...
    def declarations(self):  # 构造声明节点的方法
        ...
        while True:  # 遍历声明
            if self.current_token.type == VAR:  # 如果当前记号为变量
                ...
                self.eat(ID)  # 验证过程名称
                params = []  # 参数列表
                if self.current_token.type == LPAREN:  # 如果遇到左括号
    ...
    def formal_parameter_list(self):  # 添加创建参数列表节点的方法(循环完所有行参数)
        if self.current_token.type != ID:  # 如果没有参数
            return []  # 返回空列表
        param_nodes = self.formal_parameters()  # 添加同类型参数节点到参数节点列表
        while self.current_token.type == SEMI:  # 当前记号是分号时还有更多参数
            self.eat(SEMI)  # 验证分号
    ...
    def formal_parameters(self):  # 添加创建参数节点的方法  （循环完每一行参数）
        ...
        while self.current_token.type == COMMA:  # 当遇到逗号时
            self.eat(COMMA)  # 验证逗号
...
class DrawASTtreeImg(NodeVisitor):  # 添加符号表生成器类
    ...
    def visit_ProcedureDecl(self, node):  # 定义过程声明的绘图方法
        ...
        ProcedureDecldata = [{"children": Paramdata + Blockdata, "name": "ProcedureDecl\n" + node.name}]
    ...
    def drawastimg(self):  # 绘制AST语法树图形
        ...
        Asttree.render(r'out/L16AstTree.html')  # 绘制AST语法树图形
...
begin { Main }
    ...
    print('AST语法树图:out/L16AstTree.html')
