...
class SemanticAnalyzer(NodeVisitor):  # 添加语义分析器
    ...
    def visit_ProcedureCall(self, node):  # 过程调用遍历方法
        for param_node in node.actual_params:  # 遍历调用所有的实参列表
            self.visit(param_node)  # 但由于最底层的visit_Num等是空，所以这里的遍历暂时没有什么意义
