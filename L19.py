...
class ErrorCode(Enum):
    ...
    PROC_CALL_ERROR = 'Procedure Call Paramserror'
...
class SemanticAnalyzer(NodeVisitor):  # 添加语义分析器
    ...
    def visit_ProcedureCallold(self, node):  # 过程调用遍历方法
    ...
    def visit_ProcedureCall(self, node):  # 过程调用遍历方法
        ...
        print(proc_name, '形式参数:', proc_symbol.params)  # 从过程符号中取得该过程的形式参数列表
        print(proc_name, '实际参数:', node.actual_params)  # 从过程节点取得该过程的实际参数列表
        if (len(proc_symbol.params) == len(node.actual_params)):  # 实际参数个数和形式参数相等
            for param_node in node.actual_params:  # 遍历调用所有的实参即可
                self.visit(param_node)
        else:  # 报告过程调用参数错误
            self.error(error_code=ErrorCode.PROC_CALL_ERROR, token=node.token)
...
class DrawASTtreeImg(NodeVisitor):  # 添加符号表生成器类
    ...
    def drawastimg(self):  # 绘制AST语法树图形
        ...
        Asttree.render(r'out/L19AstTree.html')  # 绘制AST语法树图形
...
begin { Main }
   ...
   Alpha(3 + 5,7);  { procedure call }
    ...
    print('AST语法树图:out/L19AstTree.html')
