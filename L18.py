...
class Lexer(object):       #词法分析器
    ...
    def _id(self):  # 获取保留字或赋值名称记号的方法
        ...
        token= Token(TokenType.ID, result,self.lineno,self.column) if keytype is None else Token(keytype, result.upper(),self.lineno,self.column)
        return token
...
class Parser:  #语法分析器
    ...
    def statement(self):  # 添加获取语句节点的方法
        ...
        elif self.current_token.type == TokenType.ID:  # 如果遇到一个名称，说明是赋值语句。
...
class DrawASTtreeImg(NodeVisitor):  # 添加符号表生成器类
    ...
    def drawastimg(self):    # 绘制AST语法树图形
        ...
        Asttree.render(r'out/L18AstTree.html') #绘制AST语法树图形
...
begin { Main }
    ...
    print('AST语法树图:out/L18AstTree.html')
